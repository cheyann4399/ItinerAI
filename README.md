...
- `docs/` 下放：
  - `demo.gif`（建议录一个从填写表单到生成行程的 20～40 秒 GIF）
  - `screen-1.png`（需求表单）
  - `screen-2.png`（景点卡片选择）
  - `screen-3.png`（生成后的旅行图谱 + 右侧一句话修改）
...
- 我重点解决的问题：**规划效率低、需求采集遗漏、景点盲选、行程不合理、修改成本高**
- 我做的工程化工作：**前后端分层、统一数据结构（Pydantic Schema）、鉴权、可扩展的多智能体协作骨架、接口联调闭环**
- 我后续可扩展的方向：**真实地图/天气/POI 接入、导出 PDF/Excel、拖拽编辑、熔断降级与缓存**
